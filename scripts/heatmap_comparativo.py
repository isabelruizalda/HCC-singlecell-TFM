...
def main():
    ...
    for num_grupo, grupo in enumerate(grupos, start=1):
        ...
        # ── FILTRO z-score ──────────────────────────────────────
        # Mantener solo genes con z-score >= 1 en al menos un
        # tipo celular en TODOS los datasets presentes.
        # Para cada dataset, agrupamos las columnas por tipo celular
        # y comprobamos si el gen tiene al menos una columna >= 1.
        datasets_presentes = list(dict.fromkeys(ds for _, ds in col_info))
        genes_a_mantener = []
        for gen in matrix_df.index:
            cumple_todos = True
            for ds in datasets_presentes:
                # Columnas de este dataset para este gen
                cols_ds = [f"{ct}|{ds}" for ct, ds2 in col_info if ds2 == ds]
                # Filtrar columnas que existen en la matriz
                cols_ds = [c for c in cols_ds if c in matrix_df.columns]
                if not cols_ds:
                    cumple_todos = False
                    break
                # El gen debe tener z-score >= 1 en al menos un tipo celular de este dataset
        ...
        print(f"  Genes tras filtro z-score >= 1 en todos los datasets: {len(matrix_df)}/{n_antes}")
