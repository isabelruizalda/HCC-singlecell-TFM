...
def main():
    ...
    for num_grupo, grupo in enumerate(grupos, start=1):
        ...
        # ── FILTRO z-score PERMISIVO ────────────────────────────
        # Mantener genes con z-score >= 1 en AL MENOS UN dataset.
        ...
        print(f"  Genes tras filtro z-score >= 1 en al menos 1 dataset: {len(matrix_df)}/{n_antes}")
    ...
    # ── GUARDAR EXCEL CON GENES FILTRADOS ──────────────────────
    # Genera genes_filtrados_zscore.xlsx con los genes que pasaron
    # el filtro z-score >= 1, para usar en score_celltypes.py
    if grupos_data:
        registros = []
        for grupo, matrix_df, col_info in grupos_data:
            for gen in matrix_df.index:
                registros.append({"Gene": gen, "Clasificacion_final": grupo})

        genes_filtrados_df = pd.DataFrame(registros)

        # Determinar dónde guardar (mismo sitio que el Excel de genes original)
        excel_out = os.path.join(args.genes_dir, "genes_filtrados_zscore.xlsx")
        genes_filtrados_df.to_excel(excel_out, index=False)
        print(f"\n[OK] Excel de genes filtrados guardado: {excel_out}")
        print(f"     Total genes: {len(genes_filtrados_df)} en {genes_filtrados_df['Clasificacion_final'].nunique()} grupos")
    # ────────────────────────────────────────────────────────────
